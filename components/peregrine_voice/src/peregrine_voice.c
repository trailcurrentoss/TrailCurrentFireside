...
extern bool audio_play_stream_start(void);
...
/* WAV header for RIFF/PCM at 22050 Hz mono 16-bit. Total data size is
...
static void run_job(const prg_job_t *job) {
    ...
    /* Stream body → speaker, dropping the leading 44-byte WAV header. */
    if (!audio_play_stream_start()) {
    ...
    while (1) {
        ...
        size_t off_in = 0;
        if (hdr_dropped < WAV_HDR_LEN) {
            size_t need = WAV_HDR_LEN - hdr_dropped;
            size_t drop = ((size_t)n < need) ? (size_t)n : need;
            hdr_dropped += drop;
            off_in       = drop;
            if ((size_t)n <= drop) continue;
        }
        audio_play_stream_write(chunk + off_in, (size_t)n - off_in);
    }
    audio_play_stream_end();
